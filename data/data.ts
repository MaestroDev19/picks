...
async function fetchData<T>({ endpoint, page = 1, cache = 'no-store' }: FetchOptions): Promise<T> {
...
export const discoverMovie = async (page: number=1) => {
    return await fetchData<Movies[]>({ endpoint: picksEndpoints.discoverMovie, page });
};
...
export const ratedMovie = async (page: number=1) => {
    return await fetchData<Movies[]>({ endpoint: picksEndpoints.ratedMovie, page });
}

export const discoverTv = async (page: number=1) => {
    return await fetchData<TvShows[]>({ endpoint: picksEndpoints.discoverTv, page });
}
...
export const trendingMovie = async () => {
    return await fetchData<Trending[]>({ endpoint: picksEndpoints.trendingMovie});
}
...
export async function getTrailers(id: number, mediaType: 'tv' | 'movie'): Promise<Trailer[]> {
    try {
